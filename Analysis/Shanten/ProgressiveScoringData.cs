...
namespace Spines.Mahjong.Analysis.Shanten
{
  ...
  {
    public (long, int) Flags(WindScoringData windScoringData, TileType winningTile, bool isRon)
    {
      var openBit = this.OpenBit;
      ...
      this.WaitShiftValues.CopyTo(waitShiftValues);
      waitShiftValues[winningTileSuit] >>= winningTileIndex + 1;

      var suitOr = this.SuitOr;
      var suitsAnd = suitOr[0] & suitOr[1] & suitOr[2];
      var honorOr = this.HonorOr;
      var honorSum = this.HonorSum;
      ...
      waitAndRonShift += this.ShiftedAnkanCount;
      ...
      bigSum |= bigAnd & this.BigAndToSumFilter;
      ...
      result |= this.SankantsuSuukantsu & (11L << BitIndex.Sankantsu);
      ...
      var fuM = this.FuFootprint(0, keys[0]);
      var fuP = this.FuFootprint(1, keys[1]);
      var fuS = this.FuFootprint(2, keys[2]);
      ...
      var potentialBonusAnkouCountWinningSuit = (this.WaitShiftValues[winningTileSuit] >> 32) & 1L;
      var ankouFuCorrection = (potentialBonusAnkouCountWinningSuit - bonusAnkouCountWinningSuit) * ((0b100000 >> winningTileSuit) & 0b100);
      var ankouFuJihai = ((this.WaitShiftValues[3] >> 17) & (0b111L << 3)) - ankouFuCorrection;
      ...
      var fu = this.Fu + closedRonFu + tsumoFu + (int)valuePairFu + fuM + fuP + fuS + (int)ankouFuJihai + (int)singleWaitFuJihai;

...
namespace Spines.Mahjong.Analysis.State
{
  public static class AgariValidation
  {
    public static bool CanTsumo(Board board, bool isRinshanDraw)
    {
      ...
      // Most yaku
      if (ScoreLookup.Flags(seat.Hand, seat.CurrentDraw!.TileType, false, board.RoundWind.Index, seat.SeatWind.Index).Item1 != 0)
      {
        return true;
      }
    ...
    public static bool CanRon(Board board, int seatIndex)
    {
      ...
      // Most yaku
      if (ScoreLookup.Flags(hand, winningTile.TileType, true, board.RoundWind.Index, seat.SeatWind.Index).Item1 != 0)
      {
        return true;
      }
